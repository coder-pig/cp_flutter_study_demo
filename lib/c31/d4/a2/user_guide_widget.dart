...
class UserGuideWidgetState extends State<UserGuideWidget> with TickerProviderStateMixin {
  ...
  late AnimationController _floatingController; // 漂浮按钮动画控制器
  late Animation<double> _curvedAnimation; // 动画曲线

  @override
  void initState() {
    _controller = UserGuideController(this);
    _floatingController = AnimationController(vsync: this, duration: const Duration(milliseconds: 1000));
    ...
    return GestureDetector(
      ...
      child: CustomPaint(
        painter: LightPainter(
            _previousIndex == -1 ? null : widget.guidePages[_previousIndex].lightItem,
            page.lightItem,
            page.tipItem.tip,
            page.stepItem.stepButton,
            tapPosition,
            _controller,
            _curvedAnimation,
            _floatingController),
...
class LightPainter extends CustomPainter {
  ...
  final AnimationController _floatAnimation; // 漂浮按钮动画控制器

  LightPainter(this._previewItem, this._currentItem, this.tip, this.stepButton, this._tapPosition, this._controller,
      this._animation, this._floatAnimation)
      : super(repaint: Listenable.merge([_tapPosition, _animation, _floatAnimation]));
  ...
  Path _interpolatePath(Path? pathA, Path pathB, double t) {
    if (t == 1 || pathA == null) {
      ...
      final Rect interpolatedRect = Rect.fromCenter(
          center: Offset(boundsB.center.dx, boundsB.center.dy), width: boundsB.width, height: boundsB.height);
      final Path resultPath = Path();
  ...
  double paintTips(Canvas canvas) {
    ...
    double floatOffset = 10.0 * _floatAnimation.value; // 浮动偏移量
    if (isHighLightTop) {
      trianglePath.moveTo(
          lightRect.left + (lightRect.width - triangleWidth) / 2, lightRect.bottom + tipVerticalMargin + triangleWidth + floatOffset);
      trianglePath.relativeLineTo(triangleWidth / 2, -triangleWidth);
      trianglePath.relativeLineTo(triangleWidth / 2, triangleWidth);
    } else {
      trianglePath.moveTo(
          lightRect.left + (lightRect.width - triangleWidth) / 2, lightRect.top - tipVerticalMargin - triangleWidth - floatOffset);
      trianglePath.relativeLineTo(triangleWidth / 2, triangleWidth);
      trianglePath.relativeLineTo(triangleWidth / 2, -triangleWidth);
    }
    trianglePath.close();
    ...
    if (textWidth < maxTipTextWidth) {
      tipRect = Rect.fromLTWH(
          ...
          isHighLightTop
              ? lightRect.bottom + tipVerticalMargin + triangleWidth + floatOffset
              : lightRect.top - tipVerticalMargin - triangleWidth - textHeight - tipVerticalPadding * 2 - floatOffset,
